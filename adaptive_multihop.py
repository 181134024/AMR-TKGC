...
class AdaptiveMultiHopPDKGC(DisenCSPROM):
    ...
    def _score_one_group(self, sub: torch.Tensor, rel: torch.Tensor, ent_embed: torch.Tensor, text_ids=None, text_mask=None, pred_pos=None) -> tuple:
        ...
        for i in range(sub.size(0)):
            ...
            if pos_idx < 0 or pos_idx >= seq_len:
                print(f"[Warning] Adjusted pred_pos {pos_idx} exceeds sequence length {seq_len}, using last position")
                pos_idx = seq_len - 1
    ...
    def estimate_confidence_from_sub_emb(self, sub_emb_mean: torch.Tensor, rel: torch.Tensor) -> torch.Tensor:
        rel = rel.long().to(self.init_rel.device)
        rel_emb = torch.index_select(self.init_rel, 0, rel)
        conf = self.confidence_estimator(sub_emb_mean, rel_emb)
        return conf
    ...
    def forward(self, sub, rel, text_ids, text_mask, pred_pos, mode='train'):
        ...
        stop_hop = torch.zeros(B, dtype=torch.long, device=device)
        ...
        for h in range(1, self.max_hops + 1):
            ...
            sub_h = sub.index_select(0, active_idx)
            rel_h = rel.index_select(0, active_idx)
            text_ids_h = text_ids.index_select(0, active_idx) if text_ids is not None else None
            text_mask_h = text_mask.index_select(0, active_idx) if text_mask is not None else None
            pred_pos_h = pred_pos.index_select(0, active_idx) if pred_pos is not None else None

            pred_h, output_h = self._score_one_group(sub_h, rel_h, ent_embed_h, text_ids_h, text_mask_h, pred_pos_h)
            pred.index_copy_(0, active_idx, pred_h)
            output.index_copy_(0, active_idx, output_h)

            sub_emb_cur = torch.index_select(ent_embed_h, 0, sub_h).mean(dim=1)
            conf_cur = self.estimate_confidence_from_sub_emb(sub_emb_cur, rel_h)

            confidence_scores.index_copy_(0, active_idx, conf_cur)

            stop_mask = conf_cur >= self.confidence_threshold
            stopped_idx = active_idx[stop_mask]
            stop_hop.index_copy_(0, stopped_idx, torch.full_like(stopped_idx, h))
            active_idx = active_idx[~stop_mask]

            if h == self.max_hops and active_idx.numel() > 0:
                stop_hop.index_copy_(0, active_idx, torch.full_like(active_idx, h))
                active_idx = active_idx[:0]

        if self.total_queries is not None:
            self.total_queries += int(B)
            binc = torch.bincount(stop_hop, minlength=self.max_hops + 1)
            for k in range(1, self.max_hops + 1):
                self.hop_statistics[k] = self.hop_statistics.get(k, 0) + int(binc[k].item())
            self.avg_conf_sum += float(confidence_scores.sum().item())
            self.avg_conf_count += int(B)
            self.conf_min = min(self.conf_min, float(confidence_scores.min().item()))
            self.conf_max = max(self.conf_max, float(confidence_scores.max().item()))

        return pred, output, confidence_scores
...
class AdaptiveMultiHopRunner(Runner):
    ...
    def add_model(self, model_name):
        if model_name.lower() == 'adaptive_multihop':
            if self.max_hops >= 5:
                print(f"Creating adaptive multi-hop model, device: {self.device}, thresholds: hop1={self.hop1_threshold}, hop2={self.hop2_threshold}, hop3={self.hop3_threshold}, hop4={self.hop4_threshold}")
            elif self.max_hops >= 3:
                print(f"Creating adaptive multi-hop model, device: {self.device}, thresholds: hop1={self.hop1_threshold}, hop2={self.hop2_threshold}, hop3={self.hop3_threshold}")
            else:
                print(f"Creating adaptive multi-hop model, device: {self.device}, thresholds: hop1={self.hop1_threshold}, hop2={self.hop2_threshold}")
            model = AdaptiveMultiHopPDKGC(self.edge_index.to(self.device),
                                         self.edge_type.to(self.device),
                                         self.p)
            model.to(self.device)
            print("Adaptive multi-hop model created successfully")
            return model
    ...
    def run_epoch(self, epoch):
        ...
        for step, batch in enumerate(train_iter):
            try:
                ...
                lld_losses = []
                if self.p.mi_epoch > 0:
                    for _ in range(self.p.mi_epoch):
                        self.model.mi_Discs.train()
                        lld_loss = self.model.lld_best(sub, rel)
                        # Handle case when lld_loss is a float (e.g., num_factors=1)
                        if isinstance(lld_loss, (int, float)):
                            if lld_loss == 0.0:
                                # Skip backward pass when loss is zero float
                                lld_losses.append(0.0)
                                continue
                            else:
                                # Convert to tensor if needed
                                lld_loss = torch.tensor(lld_loss, device=sub.device, requires_grad=True)
                        if isinstance(lld_loss, torch.Tensor) and lld_loss.requires_grad:
                            self.optimizer_mi.zero_grad()
                            lld_loss.backward()
                            self.optimizer_mi.step()
                            lld_losses.append(lld_loss.item())
                        else:
                            lld_losses.append(float(lld_loss) if isinstance(lld_loss, torch.Tensor) else float(lld_loss))
        ...
        print(f"Training loop completed, processed {step+1} batches in total")
        try:
...
def create_adaptive_multihop_runner(params):
    ...
    runner = AdaptiveMultiHopRunner(params)
    return runner
